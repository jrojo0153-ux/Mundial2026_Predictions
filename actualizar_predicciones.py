...
def run_automated_predictions():
    ...
    try:
        ...
        # Cargamos usando el nuevo formato .keras
        m_nn = tf.keras.models.load_model('data/modelo_nn.keras') if os.path.exists('data/modelo_nn.keras') else None
    ...
    # 2. Simular obtención de partidos
    equipos = le.classes_
    df_reporte = pd.DataFrame({
        'Fecha': [pd.Timestamp.now()],
        'Consenso_Modelos': ['XGBoost+CatBoost+NN (keras) activos'],
        'Estado': ['Sincronizado con GitHub (v2)']
    })
